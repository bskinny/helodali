(ns helodali.views.pages
  (:require [helodali.db :as db]
            ...
            [re-com.core :as re-com :refer [box v-box h-box label modal-panel hyperlink single-dropdown md-icon-button
                                            input-textarea input-text row-button button title checkbox p border line]]))
      ...
      [v-box :gap "8px" :justify :start :align :start :padding "10px" ;:width "100%"
       ...
       :children [[label :width "11ch" :class "uppercase light-grey" :label "Exhibition:"]
                  [v-box :gap "2px" :align :start :justify :center ;:width "100%" ;:style {:border "dashed 1px red"}
                               :children [[label :class "italic" :label (if (not (nil? (:page-name @exhibition)))
                   ...
                   :children [[:span.uppercase.bold (str "Exhibition")]
                              ...
                              [h-box :gap "6px" :align :center
                               :children [[:span.uppercase.bold "Page Name (single word)"]
                                          [input-text :width "280px" :model (str @page-name) :style {:border "none"}
...
(defn item-view
  ...
  (let [enabled (subscribe [:by-path [:pages :enabled]])
        ;exhibitions (subscribe [:by-path [:pages :public-exhibitions]])
    ...
    (fn []
      (pprint (str "enabled: " @enabled))
      (pprint (str "exhibitions: " @exhibitions))
            ...
            view [(if @enabled
                  ...
                  (when (not (empty? @exhibitions))
                    [v-box :gap "16px" :align :start :justify :start
                     :children (into [] (mapv (fn [idx exhibition bg] ^{:key (str "exhibition-" idx)} [display-exhibition-view idx bg])
