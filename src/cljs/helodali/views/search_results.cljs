...
(defn item-list-view
  "Display item properties in single line - no image display. The 'widths' map contains the string
   ...
   The incoming 'match' is a map of the form {:type type :uuid uuid :match 'string' :title 'string'} where
   uuid points to an item of type 'type' and 'title' is something from the item that can stand for a name
   or title (e.g. Contact's name, Artwork's title)."
        ...
        (let [widths {:match 80 :title 30}
              ...
              header [h-box :align :center :justify :start :padding "8px" :width "100%"
                        :children [[hyperlink :class "bold uppercase" :style {:width "8ch"}
                                   ...
                                   [hyperlink :class "bold uppercase" :style {:width (str (max 18 (:match widths)) "ch")}
          ...
          [v-box :gap "4px" :align :center :justify :start
             :children (into [header] (map (partial item-list-view pattern widths) @matches (cycle [true false])))])))))
