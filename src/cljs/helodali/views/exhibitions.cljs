...
(defn referred-artwork-list-view
  ...
  [exhibition-uuid]
  (let [sort-key (subscribe [:by-path [:sort-keys :contacts-referred-artwork]])
        items (subscribe [:filtered-items-keys-sorted-by-key (partial exhibition-referenced-in-artwork? exhibition-uuid) :artwork :contacts-referred-artwork sort-by-key-then-created])]
    (fn []
      (if (empty? @items)
        [re-com/gap :size "1px"]
        (let [widths {:title 54}
              ...
              header [h-box :align :center :justify :start :width "100%"
                        :children [[hyperlink :class "uppercase" :style {:width (str (:title widths) "ch")} :label "Artwork Title"
                                      :tooltip "Sort by Title" :on-click #(if (= (first @sort-key) :title)
                                                                            (dispatch [:set-local-item-val [:sort-keys :contacts-referred-artwork 1] (not (second @sort-key))])
                                                                            (dispatch [:set-local-item-val [:sort-keys :contacts-referred-artwork] [:title true]]))]
                                   [hyperlink :class "uppercase" :style {:width "6ch"} :label "year"
                                       :tooltip "Sort by Year" :on-click #(if (= (first @sort-key) :year)
                                                                            (dispatch [:set-local-item-val [:sort-keys :contacts-referred-artwork 1] (not (second @sort-key))])
                                                                            (dispatch [:set-local-item-val [:sort-keys :contacts-referred-artwork] [:year false]]))]
                                   [hyperlink :class "uppercase" :style {:width "12ch"} :label "status"
                                       :tooltip "Sort by Status" :on-click #(if (= (first @sort-key) :status)
                                                                              (dispatch [:set-local-item-val [:sort-keys :contacts-referred-artwork 1] (not (second @sort-key))])
                                                                              (dispatch [:set-local-item-val [:sort-keys :contacts-referred-artwork] [:status true]]))]
                                   [hyperlink :class "uppercase" :style {:width "12ch"} :label "list price"
                                      :tooltip "Sort by Price" :on-click #(if (= (first @sort-key) :list-price)
                                                                            (dispatch [:set-local-item-val [:sort-keys :contacts-referred-artwork 1] (not (second @sort-key))])
                                                                            (dispatch [:set-local-item-val [:sort-keys :contacts-referred-artwork] [:list-price false]]))]]]]
    ...
    (fn []
      (let [view-control [h-box :gap "12px" :justify :center :align :center :margin "14px" :style {:font-size "18px"}
        ...
        [v-box :gap "10px" :align :center :justify :start ;:style {:border "dashed 1px red"}
               :children (concat (if @editing edit view-box)
                                 (if (= @display-type :new-item)
                                   [create-control]
                                   (if @editing [save-control] [view-control]))
                                 (when (not @editing)
                                   ; [[label :width "6ch" :label "Foob"]]))]))))
                                   [[referred-artwork-list-view @uuid]]))]))))
      ...
      (let [widths {:name (+ 4 (max-string-length @names 80)) ;; Adding 4 to the length for font width fudging
            ...
            header [h-box :align :center :justify :start :width "100%"
                      :children [[hyperlink :class "bold uppercase" :style {:width (str (max 18 (:name widths)) "ch")}
                                 ...
                                 [hyperlink :class "bold uppercase" :style {:width (str (max 18 (:location widths)) "ch")}
